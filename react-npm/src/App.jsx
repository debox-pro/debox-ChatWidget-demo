...
export default function App() {
    ...
    if (groupName || chainId || contractAddress) {
      try {
        const response = await fetch(
          `https://intis.top/getconversationid?group_name=${encodeURIComponent(groupName || '')}&chain_id=${encodeURIComponent(chainId || '')}&contract_address=${encodeURIComponent(contractAddress || '')}`,
          {
            headers: {
  ...
  const fetchConversationId = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `https://intis.top/getconversationid?group_name=${encodeURIComponent(group_name)}&chain_id=${encodeURIComponent(chain_id)}&contract_address=${encodeURIComponent(contract_address)}`,
        {
          headers: {
            'Content-Type': 'application/json'
          }
